...
public interface IRepository<TEntity, TId> where TEntity : class, IEntity<TId> where TId : notnull
{
    ...
    Task<IPagedResult<TEntity>> GetPagedAsync(int pageIndex, int pageSize, ISpecification<TEntity>? spec = null, bool asTracking = false, CancellationToken cancellationToken = default);
    ...
    Task<IPagedResult<TEntity>> SearchPagedAsync(string searchTerm, int pageIndex, int pageSize, ISpecification<TEntity>? spec = null, bool asTracking = false, CancellationToken cancellationToken = default);
}